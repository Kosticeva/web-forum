...
public class WarnComplaintServlet {

	//upozoravanje korisnika
	@POST
	@Path("/{id}/warn")
	@Produces(MediaType.APPLICATION_JSON)
	@Consumes(MediaType.APPLICATION_JSON)
	public String warnEntity(@PathParam("id") int id, User sender){
		...
		if(c!=null){
			...
			if(e instanceof Subforum){
				...
				msg = "Forum \""+((Subforum)e).getTitle()+"\" has been reported by \""+sender.getUsername()+"\"";
				...
				msg = "Topic\""+((Subject)e).getTitle()+"\" under forum \""+((Subject)e).getParent()
						+"\" has been reported by \""+sender.getUsername()+"\"";
			}else{
				...
				msg = "Comment\""+((Comment)e).getCommentId()+"\" at topic \""+((Comment)e).getParent()+"\"under forum \""+
						((Comment)e).getGrandparent()+"\" has been reported by \""+sender.getUsername()+"\"";
			}
